    ...
    sm:px-6 lg:px-8 py-12'>
      ...
      flex-between items-center mb-8 gap-4'>
        ...
        space-x-4'>
          ...
          hover:from-blue-600
          hover:to-indigo-700 rounded-lg
          shadow-sm hover:shadow-md
          focus:outline-none
          focus-visible:ring-2
          focus-visible:ring-indigo-500
          transition-all duration-300
          cursor-pointer' onClick={toggleExpandAll}> 
            <i className={`bx bx-${expandAll ? 
              "collapse-alt" : "expand-alt"}
              text-lg`}></i>
      ...
      dark:border-indigo-900/30 overflow-hidden
      transition-all duration-100'>
        {mpiData.map((item) => (
          <MpiItems key={item.id} item={item}
          onClick={toggleItem} 
